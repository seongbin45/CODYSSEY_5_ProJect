...
이 프로그램의 기본값은 코드에 박힌 `gemini-2.5-flash`입니다. 그 이름이 사라지거나 `generateContent`와 맞지 않게 바뀌면, 코드를 고치지 않고는 채점 경로를 유지할 수 없습니다. 업데이트된 모델 목록을 다시 불러오는 로직을 만들지 않는 한 유지보수가 불가능합니다.
